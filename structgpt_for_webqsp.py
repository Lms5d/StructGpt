...
class ChatGPT:
    def __init__(self, args, prompt_path, prompt_name, max_tokens):
        ...
        # self.tokenizer = LlamaTokenizer.from_pretrained("meta-llama/Llama-3.2-3B-Instruct")
        # self.model = LlamaForCausalLM.from_pretrained("meta-llama/Llama-3.2-3B-Instruct")
    ...
    def  _v1(self, input_text, turn_type, tpe_name=None):
        if self.args.debug:
    ...
    #     # 使用 Llama 模型生成回答
    #     inputs = self.tokenizer(input_text, return_tensors="pt", truncation=True, padding=True, max_length=2048)
